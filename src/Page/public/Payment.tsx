...
// 確認付款並通知 mapStore
const confirmPaymentOnMap = async (
  city: string,
  country: string,
  product: MapProduct
) => {
...
const Payment = () => {
  ...
  const msg =
    orderStore?.orderData?.message || userStore.userInfo?.data?.message;
      ...
      if (country && city && firstItem) {
        const mapProduct: MapProduct = {
          ...
          author: (() => {
            try {
              return (
                JSON.parse(firstItem.product.content || '{}').author ||
                'Unknown'
              );
            } catch {
              return 'Unknown';
                  ...
                  {cartItems.map((item) => (
                    <div key={item.id} className="p-4 flex gap-4 items-start">
                      <img
                        ...
                        className="w-20 h-28 object-cover rounded shadow-sm border border-border"
                      />
                ...
                <div className="p-6 space-y-4">
                  ...
                  <div className="pt-4 border-t border-border mt-4 flex justify-between items-center">
                    ...
                    <span className="text-2xl font-bold text-primary">
                      $
                      {Math.round(
                        orderFinalTotal || cartsData.data?.final_total || 0
                      )}{' '}
                      NTD
                    </span>
                  </div>
